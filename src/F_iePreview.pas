...
{$ifdef FPC}
  ...
  LCLType,
{$endif}
  Windows, SysUtils, Classes, Variants, Graphics, Controls, Forms, Generics.Collections,
  Dialogs, StdCtrls, ComCtrls, ExtCtrls, Buttons, SHDocVw, Utf8IniFiles,
...
type
  ...
  TFrmIEPreview = class(TNppDockingForm)
    wbIE: TWebBrowser;
    pnlButtons: TPanel;
    btnRefresh: TButton;
    btnClose: TButton;
...
var
  frmIEPreview: TFrmIEPreview;

////////////////////////////////////////////////////////////////////////////////////////////////////
implementation
uses
  ShellAPI, ComObj, StrUtils, Masks, MSHTML,
  RegExpr,
  Registry,
  Debug,
{$ifndef FPC}
...
begin
  ...
  begin
    ...
    KillTimer(frmIEPreview.Handle, EventID);
  end;
end;

{$ifndef FPC}
function WideStringReplace(const S: string; const SubStr,Replacement: string; Flags: TReplaceFlags): string;
begin
  Result := StringReplace(S, SubStr, Replacement, Flags);
end;
{$endif}
...
constructor TFrmIEPreview.Create(AOwner: TComponent);
{$ifndef FPC}
var
...
procedure TFrmIEPreview.ToggleDarkMode;
{$ifndef FPC}
begin
end;
{$else}
...
procedure TFrmIEPreview.SubclassAndTheme(DmfMask: Cardinal);
begin
...
procedure TFrmIEPreview.sbrIEDrawPanel(StatusBar: TStatusBar; Panel: TStatusPanel; const Rect: TRect);
var
  Palette: TDarkModeColors;
  H, X, Y: Integer;
begin
...
{ ------------------------------------------------------------------------------------------------ }
procedure TFrmIEPreview.FormCreate(Sender: TObject);
begin
...
end {TFrmIEPreview.FormCreate};
{ ------------------------------------------------------------------------------------------------ }
procedure TFrmIEPreview.FormDestroy(Sender: TObject);
begin
  FreeAndNil(FScrollPositions);
  FreeAndNil(FFilterThread);
  inherited;
end {TFrmIEPreview.FormDestroy};


{ ------------------------------------------------------------------------------------------------ }
procedure TFrmIEPreview.btnCloseStatusbarClick(Sender: TObject);
begin
  sbrIE.Visible := False;
end;

{ ------------------------------------------------------------------------------------------------ }
procedure TFrmIEPreview.tmrAutorefreshTimer(Sender: TObject);
begin
  tmrAutorefresh.Enabled := False;
  btnRefresh.Click;
end {TFrmIEPreview.tmrAutorefreshTimer};
...
end {TFrmIEPreview.btnRefreshClick};

{ ------------------------------------------------------------------------------------------------ }
procedure TFrmIEPreview.chkFreezeClick(Sender: TObject);
begin
  btnRefresh.Enabled := not chkFreeze.Checked;
  if btnRefresh.Enabled then
    btnRefresh.Click;
end {TFrmIEPreview.chkFreezeClick};

{ ------------------------------------------------------------------------------------------------ }
procedure TFrmIEPreview.DisplayPreview(const BufferID: TBufferID);
var
  IsHTML: Boolean;
  HeadStart: Integer;
  Size: WPARAM;
...
ODS('DisplayPreview(HTML: "%s"(%d); BufferID: %x)', [StringReplace(Copy({$ifdef FPC}UTF8Encode{$endif}(HTML), 1, 10), #13#10, '', [rfReplaceAll]), Length(HTML), BufferID]);
      ...
      if wbIE.GetDocument <> nil then
        self.UpdateDisplayInfo({$ifdef FPC}UTF8Encode{$endif}(wbIE.GetDocument.title))
      else
    ...
    pnlButtons.Enabled := not Self.AlphaBlend;
...
end {TFrmIEPreview.DisplayPreview};

{ ------------------------------------------------------------------------------------------------ }
procedure TFrmIEPreview.SaveScrollPos;
var
  docEl: IHTMLElement2;
  P: TPoint;
begin
...
end {TFrmIEPreview.SaveScrollPos};

{ ------------------------------------------------------------------------------------------------ }
procedure TFrmIEPreview.RestoreScrollPos(const BufferID: TBufferID);
var
  P: TPoint;
  docEl: IHTMLElement2;
begin
...
begin
  ...
  ResetTimer;
end {TFrmIEPreview.ForgetBuffer};
...
end {TFrmIEPreview.ReloadSettings};

{ ------------------------------------------------------------------------------------------------ }
procedure TFrmIEPreview.ResetTimer;
begin
  tmrAutorefresh.Enabled := False;
  tmrAutorefresh.Enabled := True;
end {TFrmIEPreview.ResetTimer};

{ ------------------------------------------------------------------------------------------------ }
function TFrmIEPreview.DetermineCustomFilter: string;
var
...
end {TFrmIEPreview.DetermineCustomFilter};

{ ------------------------------------------------------------------------------------------------ }
function TFrmIEPreview.ExecuteCustomFilter(const FilterName: string; const HTML: WideString; const BufferID: TBufferID): Boolean;
var
...
end {TFrmIEPreview.ExecuteCustomFilter};

{ ------------------------------------------------------------------------------------------------ }
procedure TFrmIEPreview.FilterThreadTerminate(Sender: TObject);
begin
...
{ ------------------------------------------------------------------------------------------------ }
procedure TFrmIEPreview.btnAboutClick(Sender: TObject);
begin
  (npp as TNppPluginPreviewHTML).CommandShowAbout;
end {TFrmIEPreview.btnAboutClick};

{ ------------------------------------------------------------------------------------------------ }
procedure TFrmIEPreview.btnCloseClick(Sender: TObject);
begin
  self.Hide;
end {TFrmIEPreview.btnCloseClick};
...
procedure TFrmIEPreview.FormKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TFrmIEPreview.FormHide(Sender: TObject);
begin
  SaveScrollPos;
...
{ ------------------------------------------------------------------------------------------------ }
procedure TFrmIEPreview.FormDock(Sender: TObject);
begin
  ResetTimer;
end;

{ ------------------------------------------------------------------------------------------------ }
procedure TFrmIEPreview.FormFloat(Sender: TObject);
begin
  if pnlButtons.Enabled then
    ResetTimer;
end;

{ ------------------------------------------------------------------------------------------------ }
procedure TFrmIEPreview.FormShow(Sender: TObject);
begin
  inherited;
  ToggleDarkMode;
  ReloadSettings;
...
{ ------------------------------------------------------------------------------------------------ }
function TFrmIEPreview.TransformXMLToHTML(const XML: WideString): WideString;
  { - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - }
...
end {TFrmIEPreview.TransformXMLToHTML};

{ ------------------------------------------------------------------------------------------------ }
procedure TFrmIEPreview.wbIEBeforeNavigate2(ASender: TObject; const pDisp: IDispatch; const URL,
...
{ ------------------------------------------------------------------------------------------------ }
procedure TFrmIEPreview.wbIEDocumentComplete(ASender: TObject; const pDisp: IDispatch;
...
end {TFrmIEPreview.wbIEDocumentComplete};

{ ------------------------------------------------------------------------------------------------ }
procedure TFrmIEPreview.wbIENewWindow3(ASender: TObject; var ppDisp: IDispatch;
  var Cancel: WordBool; dwFlags: Cardinal; const bstrUrlContext, bstrUrl: WideString);
var
  Handle: HWND;
begin
  if not WideSameText(bstrUrl, 'about:blank') and not WideSameText('javascript:', Copy(bstrURL, 1, 11)) then begin
    if Assigned(Npp)  then
      Handle := Npp.NppData.NppHandle
    else
      Handle := 0;
    ShellExecuteW(Handle, nil, PWideChar(bstrUrl), nil, nil, SW_SHOWDEFAULT);
  end;
  Cancel := True;
end;

{ ------------------------------------------------------------------------------------------------ }
procedure TFrmIEPreview.wbIEStatusBar(ASender: TObject; StatusBar: WordBool);
begin
  sbrIE.Visible := StatusBar;
end;

{ ------------------------------------------------------------------------------------------------ }
procedure TFrmIEPreview.wbIEStatusTextChange(ASender: TObject; const Text: WideString);
begin
...
{ ------------------------------------------------------------------------------------------------ }
procedure TFrmIEPreview.wbIETitleChange(ASender: TObject; const Text: WideString);
begin
  inherited;
  self.UpdateDisplayInfo({$ifdef FPC}UTF8Encode{$endif}(WideStringReplace(Text, 'about:blank', '', [rfReplaceAll])));
end;
